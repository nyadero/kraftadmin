...
import com.bowerzlabs.formfields.fields.ColorField;
...
public class ColorFieldStrategy implements FormFieldStrategy {
    @Override
    public boolean supports(Field field, DbObjectSchema dbObjectSchema) {
        if (field.isAnnotationPresent(FormInputType.class)) {
            ...
            return formInputType.value().equals(FormInputType.Type.COLOR);
        }
        return false;
    }
    ...
    public FormField createField(Field field, DbObjectSchema dbObjectSchema, String inputName, boolean isSearch, List<EntityType<?>> subTypes) {
        ...
        return new ColorField(label, placeholder, required, (String) value, inputName, dbObjectSchema.getValidationErrors(), dbObjectSchema.getValidationRules());
    }
}
