...
public class CheckboxFieldStrategy implements FormFieldStrategy {
    @Override
    public boolean supports(Field field, DbObjectSchema dbObjectSchema) {
        if (field.isAnnotationPresent(FormInputType.class)) {
            ...
            return formInputType.value().equals(FormInputType.Type.CHECKBOX);
        }
    ...
    public FormField createField(Field field, DbObjectSchema dbObjectSchema, String inputName, boolean isSearch, List<EntityType<?>> subTypes) {
        ...
        return new CheckboxField(label, "true".equalsIgnoreCase(String.valueOf(value)), inputName, required, dbObjectSchema.getValidationErrors(), dbObjectSchema.getValidationRules());
    }
}
