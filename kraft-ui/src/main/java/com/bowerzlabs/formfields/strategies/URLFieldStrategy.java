...
import com.bowerzlabs.formfields.fields.URLField;
...
public class URLFieldStrategy implements FormFieldStrategy {
    @Override
    public boolean supports(Field field, DbObjectSchema dbObjectSchema) {
        if (field.isAnnotationPresent(FormInputType.class)) {
            ...
            return formInputType.value().equals(FormInputType.Type.URL);
        }
        return false;
    }
    ...
    public FormField createField(Field field, DbObjectSchema dbObjectSchema, String inputName, boolean isSearch, List<EntityType<?>> subTypes) {
        ...
        return new URLField(label, placeholder, required, (String) value, inputName, dbObjectSchema.getValidationRules(), dbObjectSchema.getValidationErrors());
    }
}
