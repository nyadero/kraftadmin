...
import com.bowerzlabs.formfields.fields.DateTimeField;
...
public class DateTimeFieldStrategy implements FormFieldStrategy {
    @Override
    public boolean supports(Field field, DbObjectSchema dbObjectSchema) {
        if (field.isAnnotationPresent(FormInputType.class)) {
            ...
            return formInputType.value().equals(FormInputType.Type.DATETIME);
        }
    ...
    public FormField createField(Field field, DbObjectSchema dbObjectSchema, String inputName, boolean isSearch, List<EntityType<?>> subTypes) {
        ...
        return new DateTimeField(label, placeholder, required, (String) value, inputName, dbObjectSchema.getValidationErrors(), dbObjectSchema.getValidationRules());
    }
}
