...
public class KraftSecurityConfig {
    ...
    @Bean
    public ServletContextInitializer kraftSessionTimeoutInitializer() {
        return servletContext -> {
            servletContext.getSessionCookieConfig().setMaxAge(30 * 60); // 30 minutes in seconds
            servletContext.setSessionTimeout(30); // 30 minutes
        };
    }

    @Bean
    public ApplicationListener<ContextRefreshedEvent> clearSessionsOnRestart(SessionRegistry sessionRegistry) {
        return event -> {
